...
altpllSync clkIn rstIn = clocksResetSynchronizer (unsafeAltpll clkIn rstIn) clkIn
...
-- synchronized before it can be used as a (reset) signal. The domain @pllLock@
-- is left completely free. If the lock signal is to be used in multiple
-- domains, the @pllLock@ domain should probably be set to @domIn@. While in HDL
